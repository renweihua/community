...
class UserService extends Service
{
    ...
    public static function setUser($socket, array $user): bool
    {
        ...
        $res1 = $redis->hSet(self::USERS_LIST, $sid, my_json_encode(['user_id' => $user['user_id'], // 会员Id
                                                                                              'nick_name' => $user['nick_name'], // 昵称
                                                                                              'user_avatar' => $user['user_avatar'], // 头像：可访问的URL地址
                                                                                          ]));
        // 会员Id与sid列表，方便通过sid获取到会员Id
        $res2 = $redis->hSet(self::USER_SID, (string)$user_id, $sid);
