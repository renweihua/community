...
class CreateUsersTable extends Migration
{
    ...
    public function up()
    {
        ...
        Schema::create('users', function (Blueprint $table) {
            ...
            $table->string('user_name', 256)->default('')->comment('用户名');
            $table->string('user_email', 256)->default('')->comment('邮箱');
            $table->string('password', 60)->default('')->comment('登录密码');
            $table->string('oauth_password', 256)->default('')->comment('用于第三方登录【如果账户修改了登录密码，jwt登录【需要账户与密码才可以】，hash加密又无法解密，将导致无法快捷与账户登录。】');
