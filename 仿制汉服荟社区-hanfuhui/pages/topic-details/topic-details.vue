    ...
    <view>
        ...
        <mescroll-uni :down="{use:false}" :up="{onScroll:true}" @up="upCallback" @scroll="scroll" @init="mescrollInit">
            ...
            <view class="plr18r ptb18r bgwhite mb18r">
                ...
                <!-- 内容 -->
                <view class="fword f28r c111" :class="{mb18r: calImageSrcs }" v-if="topicInfoData.Describe">
                    {{topicInfoData.Describe}}
                </view>
                <!-- 图片格 -->
                ...
                <view class="flexr-jsb flex-aic ptb18r">
                    ...
                    <view class="bgwhite f28r ctheme ball2r-ctheme fcenter w128r br8r ptb8r" @tap="fnTopicFollow">
                        {{topicInfoData.TopicFollow?'已关注':'关注'}}
                    </view>
                </view>
            </view>
            ...
            <view id="tabbar" class="flexr-jsa flex-ais hl90r bgwhite bbs2r" :class="{'tabbar-fixed':isFixed}">
                <view class="f32r fbold fcenter w128r tabbar" :class="{'tabbarsh':current == 0}" @tap="fnBarClick(0)">
                    最热
                </view>
                <view class="f32r fbold fcenter w128r tabbar" :class="{'tabbarsh':current == 1}" @tap="fnBarClick(1)">
                    最新
                </view>
            </view>
            <!-- 最热 -->
            <view v-if="status.hottest" :style="{display: current==0 ? 'block' :'none'}">
                <block v-for="(infoData,index) in topicHottestListData" :key="index">
                    <trend-card :info-data="infoData" @click="fnCardInfo" @user="fnCardUser" @top="fnCardTop"
                                @comm="fnCardComm"
                                @save="fnCardSave" @follow="fnCardFollow" @black="fnCardBlack"
