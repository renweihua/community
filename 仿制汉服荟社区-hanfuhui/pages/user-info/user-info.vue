	...
	<view>
		...
		<mescroll-uni :down="{ use: false }" :up="{ onScroll: true }" @up="upCallback" @scroll="scroll" @init="mescrollInit">
			...
			<image
				class="info-cover"
				@tap="fnMainBgPic"
				:src="tempUserInfoData.background_cover ? tempUserInfoData.background_cover : '/static/default_image.png'"
				mode="aspectFill"
			></image>
			<view class="plr28r pb18r bgwhite">
				...
				<view class="posir flexr-jfe flex-aic ptb18r">
					<view class="info-avatar">
						<user-avatar
							:src="tempUserInfoData.user_info.user_avatar ? tempUserInfoData.user_info.user_avatar : '/static/default_avatar.png'"
							tag=""
							size="lg"
						></user-avatar>
					</view>
					<view class="ball2r-ctheme f28r ctheme fcenter w128r br8r ptb8r mr28r" v-if="!calIsLoginUser" @tap="fnUserChat">勾搭TA</view>
					<view class="bgtheme f28r cwhite fcenter w128r br8r ptb8r" v-if="!calIsLoginUser" @tap="fnUserFollow">
						{{ tempUserInfoData.user_info.is_follow ? '已关注' : '关注' }}
					</view>
				...
				<view class="flex flex-aic mb18r">
					...
					<i-icon
						v-if="[0, 1].indexOf(tempUserInfoData.user_info.user_sex) > -1"
						:type="tempUserInfoData.user_info.user_sex_text == '男' ? 'nan' : 'nv'"
						size="28"
						:color="tempUserInfoData.user_info.user_sex_text == '男' ? '#479bd4' : '#FF6699'"
					></i-icon>
				...
				<!-- 粉丝关注人气统计 -->
				<view>
					<text class="f28r fbold c555 mr18r">{{ tempUserInfoData.FansCount || 0 }}</text>
					<text class="f24r cgray mr28r">粉丝</text>
					<text class="f28r fbold c555 mr18r">{{ tempUserInfoData.user_info.follows_count || 0 }}</text>
					<text class="f24r cgray mr28r">关注</text>
					<text class="f28r fbold c555 mr18r">{{ tempUserInfoData.Popularity || 0 }}</text>
					<text class="f24r cgray">人气</text>
				</view>
			</view>
			<!-- 选择导航 -->
			...
			<view id="tabbar" class="flexr-jsa flex-ais hl90r bgwhite mb18r" :class="{ 'tabbar-fixed': isFixed }">
				<view class="f32r fbold fcenter w128r tabbar" :class="{ tabbarsh: current == 0 }" @tap="fnBarClick(0)">他的动态</view>
			...
			<view v-if="status.publish" :style="{ display: current == 0 ? 'block' : 'none' }">
				<block v-for="(infoData, index) in userPublishListData" :key="index">
					<trend-card :head="false" :item="infoData" @click="fnCardInfo" @huiba="fnCardHuiba" @top="fnCardTop" @comm="fnCardComm" @save="fnCardSave"></trend-card>
				</block>
			</view>
			<!-- 赞过 -->
...
import { getUserPublishList, getUserTopList, getUserInfo, modifyUserMainBgPic, followUser } from '@/api/UserServer.js';
...
export default {
	...
	computed: {
		...
		// 计算修改地址逗号换空格
		calAddress() {
			let addr = this.tempUserInfoData.user_info.city_info;
	...
	methods: {
		...
		upCallback(mescroll) {
			...
			// 正常加载数据
			[getUserPublishList, getUserTopList]
				[this.current](params)
				.then(res => {
					let lists = res.data;
					// 发布
					if (this.current == 0) {
						if (mescroll.num == 1) {
							this.$store.commit('user/setUserPublishListData',lists.data);
						} else {
							this.$store.commit('user/setUserPublishListData', this.userPublishListData.concat(lists.data));
						}
					}
					// 赞过
					if (this.current == 1) {
						if (mescroll.num == 1) {
							this.$store.commit('user/setUserTopListData', lists.data);
						} else {
							this.$store.commit('user/setUserTopListData', this.userTopListData.concat(lists.data));
						}
					}
