    ...
    <mescroll-uni :down="{use:false}" :up="{onScroll:true}" @up="upCallback" @scroll="scroll" @init="mescrollInit">
      ...
      <view class="plr28r pb18r bgwhite">
        ...
        <view class="posir flexr-jfe flex-aic ptb18r">
          <view class="info-avatar">
            <user-avatar :src="tempUserInfoData.HeadUrl ?tempUserInfoData.HeadUrl + '_200x200.jpg' : '/static/default_avatar.png'"
              :tag="tempUserInfoData.AuthenticateCode" size="lg"></user-avatar>
          </view>
          ...
          <view class="bgtheme f28r cwhite fcenter w128r br8r ptb8r" v-if="!calIsLoginUser" @tap="fnUserFollow">{{tempUserInfoData.UserAtte?'已关注':'关注'}}</view>
        ...
        <view class="flex flex-aic mb18r">
          ...
          <i-icon :type="tempUserInfoData.Gender == '男' ?'nan':'nv' " size="28" :color="tempUserInfoData.Gender == '男' ?'#479bd4':'#FF6699'"></i-icon>
          <text class="f24r cgray ml28r">小荟号：{{tempUserInfoData.UserName || 0}}</text>
        </view>
        <!-- 简介说明 -->
        ...
        <view>
          ...
          <text class="f28r fbold c555 mr18r">{{tempUserInfoData.AtteCount || 0}}</text><text class="f24r cgray mr28r">关注</text>
      ...
      <view v-if="status.publish" :style="{display: current==0 ? 'block' :'none'}">
        <block v-for="(infoData,index) in userPublishListData" :key="index">
          <trend-card :head="false" :info-data="infoData" @click="fnCardInfo" @huiba="fnCardHuiba" @top="fnCardTop"
            @comm="fnCardComm" @save="fnCardSave"></trend-card>
        </block>
      </view>
      <!-- 赞过 -->
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      fnUserEdit() {
        uni.navigateTo({
          url: `/pages/user-info-modify/user-info-modify?id=${this.tempUserInfoData.ID}`
        })
      },
      /// 用户跳转聊天页
      fnUserChat() {
      ...
      fnCardTop(e) {
        let filItem = {};
        // 发布
        if (this.current == 0) filItem = this.userPublishListData.filter(item => item.ObjectID == e.ObjectID)[0];
        // 赞过
