    ...
    <view>
        ...
        <view class="posif posi-tlr0 z500 bgtheme">
            ...
            <view class="flex plr18r">
                <view class="w128r hl90r f32r fcenter cwhite-a6" :class="{'barsh-home': current == 0}"
                      @tap="fnBarClick(0)">推荐
                </view>
                <view class="w128r hl90r f32r fcenter cwhite-a6" :class="{'barsh-home':current == 1}"
                      @tap="fnBarClick(1)">关注
                </view>
                <view class="w128r hl90r f32r fcenter cwhite-a6" :class="{'barsh-home':current == 2}"
                      @tap="fnBarClick(2)">广场
                </view>
            </view>
        </view>
        ...
        <swiper class="posia posi-all0" :current="current" @change="fnBarClick">
            <!-- 推荐 -->
            <swiper-item>
                <mescroll-uni v-if="status.recommend"
                              :top="90"
                              :bottom="112"
                              @down="downCallback"
                              @up="upCallback"
                              @init="mescrollInit">
                    <block v-for="(infoData,index) in mainData" :key="index">
                        <trend-card :info-data="infoData" @click="fnCardInfo" @user="fnCardUser" @huiba="fnCardHuiba"
                                    @top="fnCardTop"
                                    @comm="fnCardComm" @save="fnCardSave" @follow="fnCardFollow" @black="fnCardBlack"
                                    @report="fnCardReport"></trend-card>
                    </block>
                </mescroll-uni>
            </swiper-item>
            ...
            <!-- 关注 -->
            <swiper-item>
                <mescroll-uni v-if="status.follow" :top="90" :bottom="112" @down="downCallback" @up="upCallback"
                              @init="mescrollInit">
            ...
            <!-- 广场 -->
            <swiper-item>
                <mescroll-uni v-if="status.square" :top="90" :bottom="112" @down="downCallback" @up="upCallback"
                              @init="mescrollInit">
        ...
        methods: {
            ...
            upCallback(mescroll) {
                ...
                [getMainList, getAtteList, getSquareList][this.current]({
                    page: mescroll.num,
                    last_id: this.last_id[this.current],
                    limit: mescroll.size
                }).then(res => {
                    this.last_id[this.current] = res.data.last_id;
                    // 不同标签存入不同数据变量
                    if (this.current == 0) {
            ...
            fnCardTop(e) {
                ...
                let params = {
                    object_type: filItem.object_type,
                    dynamic_id: filItem.dynamic_id
                };
                // 用户是否点过赞
