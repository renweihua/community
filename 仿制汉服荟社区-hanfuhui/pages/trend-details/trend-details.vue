	...
	<view>
			...
			<view class="mb18r bgwhite">
					...
					<view class="flex flex-fww" v-if="calImageSrcs">
						<block v-for="(img, index) in calImageSrcs" :key="index">
							<image
								...
								:class="{
									mlr05v: index == 1 || index == 4 || index == 7,
									mb05v: (index == 1 && calImageSrcs.length > 3) || (index == 4 && calImageSrcs.length > 6)
								}"
				...
				<view class="flexr-jfe flex-aic ptb18r plr18r bts2r br8r" v-if="topListData">
					...
					<view class="f24r c111 fcenter bgf8 w128r ptb18r" @tap="fnTopList">
						...
						<text class="f24r cbrown ml18r">{{ dynamic.cache_extends ? dynamic.cache_extends.praise_count : 0}}</text>
					</view>
				</view>
			</view>
			<!-- 评论区 -->
		...
		<view class="posif posi-blr0 flexr-jsa plr18r ptb18r bts2r z5 bgwhite">
			...
			<view class="plr28r bls2r brs2r" @tap="fnTop">
				...
				<text class="f28r cgray ml8r">{{ dynamic.cache_extends ? dynamic.cache_extends.praise_count : 0 }}</text>
			</view>
	...
	methods: {
		...
		fnTop() {
			...
			if (this.fromPage == 'home') {
				// 推荐
				if (this.current == 0) filItem = this.$store.getters['trend/getMainData'].filter(item => item.dynamic_id == this.trendData.dynamic_id)[0];
				// 关注
				if (this.current == 1) filItem = this.$store.getters['trend/getAtteData'].filter(item => item.dynamic_id == this.trendData.dynamic_id)[0];
				// 广场
				if (this.current == 2) filItem = this.$store.getters['trend/getSquareData'].filter(item => item.dynamic_id == this.trendData.dynamic_id)[0];
			}
			// 来自用户详情
			if (this.fromPage == 'userinfo') {
				// 发布
				if (this.current == 0) filItem = this.$store.getters['user/getUserPublishListData'].filter(item => item.dynamic_id == this.trendData.dynamic_id)[0];
				// 赞过
				if (this.current == 1) filItem = this.$store.getters['user/getUserTopListData'].filter(item => item.dynamic_id == this.trendData.dynamic_id)[0];
			}
			// 用户是否已经点过赞
		...
		fnAtte(e) {
			...
			if (e.is_follow) {
				followUser(e.user_id).then(res => {
					...
					if (!res.status) return;
					this.dynamic.user_info.is_follow = false;
					// 来自主要跳转
					if (this.fromPage == 'home') {
				...
				followUser(e.user_id).then(res => {
					...
					if (!res.status) return;
					this.dynamic.user_info.is_follow = true;
					// 来自主要跳转
					if (this.fromPage == 'home') {
					...
					switch (res.tapIndex) {
						...
						case 3:
							...
							delComment(e.comment_id).then(res => {
								...
								if (this.fromPage == 'home') {
									// 推荐
									if (this.current == 0) filItem = this.$store.getters['trend/getMainData'].filter(item => item.dynamic_id == this.dynamic.dynamic_id)[0];
									// 关注
									if (this.current == 1) filItem = this.$store.getters['trend/getAtteData'].filter(item => item.dynamic_id == this.dynamic.dynamic_id)[0];
									// 广场
									if (this.current == 2) filItem = this.$store.getters['trend/getSquareData'].filter(item => item.dynamic_id == this.dynamic.dynamic_id)[0];
								}
								// 来自用户详情
								if (this.fromPage == 'userinfo') {
									// 发布
									if (this.current == 0)
										filItem = this.$store.getters['user/getUserPublishListData'].filter(item => item.dynamic_id == this.dynamic.dynamic_id)[0];
									// 赞过
