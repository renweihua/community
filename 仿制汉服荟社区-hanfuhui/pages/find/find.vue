    ...
    <swiper class="posia posi-all0" :current="current" @change="fnBarClick">
        ...
        <mescroll-uni v-if="status.album" :top="90" :bottom="112" @down="downCallback" @up="upCallback" @init="mescrollInit">
          ...
          <view class="flex plr18r ptb18r bgwhite mb18r">
            <view class="flex-fitem posir br8r rank-ho200r mr18r" @tap="fnAlbumRank(1)">
              <image style="width: 100%;height: 100%;" :src="rankFaceData.week ? rankFaceData.week +'_200x200.jpg/format/webp' :'/static/default_image.png'"
            ...
            <view class="flex-fitem posir br8r rank-ho200r" @tap="fnAlbumRank(2)">
              <image style="width: 100%;height: 100%;" :src="rankFaceData.month ? rankFaceData.month +'_200x200.jpg/format/webp':'/static/default_image.png'"
                mode="aspectFill" :lazy-load="true"></image>
      ...
      <swiper-item>
        <mescroll-uni v-if="status.word" :top="90" :bottom="112" @down="downCallback" @up="upCallback" @init="mescrollInit">
          <block v-for="(word,index) in wordListData" :key="index">
            <word-card :info-data="word" @user="fnUserInfo" @click="fnWordInfo"></word-card>
          </block>
        </mescroll-uni>
      </swiper-item>
      ...
      <swiper-item v-if="false">
        <mescroll-uni v-if="status.org" :top="90" :bottom="112" @down="downCallback" @up="upCallback" @init="mescrollInit">
          ...
          <block v-for="(org,index) in orgListData" :key="index">
            <org-card :info-data="org" @click="fnOrgInfo"></org-card>
          </block>
        </mescroll-uni>
      </swiper-item>
    </swiper>
  ...
  export default {
    ...
    methods: {
      ...
      upCallback(mescroll) {
        ...
        // 多组请求数据保存分类
        listData[this.current](params).then(res => {
          // 摄影
          if (this.current == 1) {
            if (mescroll.num == 1) {
