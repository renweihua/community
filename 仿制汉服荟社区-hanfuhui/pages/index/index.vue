<template>
    <view>
        <!-- 主页 -->
        <home v-if="status.home" :style="{display: current==0 ? 'block' :'none'}"
              :refresh="current==0 && clickRefresh"></home>
        <!-- 发现 -->
        <find v-if="status.find" :style="{display: current==1 ? 'block' :'none'}"
              :refresh="current==1 && clickRefresh"></find>
        <!-- 商城 -->
        <shop v-if="status.shop" :style="{display: current==3 ? 'block' :'none'}"
              :refresh="current==3 && clickRefresh"></shop>
        <!-- 我的 -->
        <my v-if="status.my" :style="{display: current==4 ? 'block' :'none'}"
            :refresh="current==4 && clickRefresh"></my>
