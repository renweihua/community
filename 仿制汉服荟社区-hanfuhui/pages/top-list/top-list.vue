        ...
        <mescroll-uni @down="downCallback" @up="upCallback">
            <block v-for="(item,index) in topListData" :key="index">
                <view class="flex plr18r ptb28r bbs2r" @tap="fnUserInfo(item.User)">
                    <user-avatar :src="item.user_info.user_head" :tag="item.user_info.uuid" size="md"></user-avatar>
                    <view class="flexc-jsa ml28r">
                        <view>
                            <text class="f28r fbold mr18r">{{item.user_info.nick_name}}</text>
                            <i-icon :type="item.user_info.user_sex_text == '男' ?'nan':'nv' " size="28"
        ...
        methods: {
            ...
            upCallback(mescroll) {
                getTopList({
                    ...
                    this.last_id = topRes.data.last_id;
