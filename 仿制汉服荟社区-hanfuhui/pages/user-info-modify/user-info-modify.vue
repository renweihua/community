	...
	<view>
		...
		<view class="mlr64r">
			...
			<view class="flexr-jsc flex-aic bbs2r h112r">
				...
				<input class="flex-fitem ml28r f32r c555" type="text" v-model="info.nick_name" placeholder="请输入昵称"
					:maxlength="10" />
			</view>
			<!-- 性别 -->
			<view class="flexr-jsc flex-aic bbs2r h112r">
				...
				<view class="flex-fitem ml28r flex hl80r">
					<view class="pr28r mr28r" @tap="fnGender(1)">
						<i-icon :type="info.user_sex == 1 ? 'quan-dui' : 'quan'" size="42"
							:color="info.user_sex == 1 ? '#FF6699' : '#555555'"></i-icon>
						<text class="f32r ml8r" :class="[info.user_sex == 1 ? 'ctheme' : 'c555']">帅汉子</text>
					</view>
					<view class="pr28r" @tap="fnGender(2)">
						<i-icon :type="info.user_sex == 2 ? 'quan-dui' : 'quan'" size="42"
							:color="info.user_sex == 2 ? '#FF6699' : '#555555'"></i-icon>
			...
			<view class="flexr-jsc flex-aic bbs2r edit-hm112r" v-if="isShowArea">
				...
				<textarea class="flex-fitem mlr28r ptb8r f32r c555 edit-hmo250r z5" placeholder="请输入简介"
					placeholder-style="font-size: 14px;color:#8F8F94" auto-height
					v-model="info.basic_extends.user_introduction"></textarea>
			</view>
		</view>
		<!-- 提交按钮 -->
		<button class="btn-sub mt64r mlr64r" hover-class="btn-hover" :disabled="isModify" :loading="isModify"
			@tap="fnModify">提交修改</button>
	</view>
</template>
	...
	export default {
		...
		methods: {
			...
			fnModify() {
				...
				// 修改简介
				if (user_introduction != this.tempInfo.basic_extends.user_introduction) {
					paramArray.push({
						option: '5',
					...
					paramArray.map(item => {
						modifyUserInfo(this.info).then(res => {
							...
							if (res.status) {
								...
								userInfo.user_info.user_introduction = this.info.basic_extends.user_introduction;
							} else {
								return;
							}
						});
