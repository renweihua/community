	...
	<view>
		...
		<mescroll-uni :bottom="90" :down="{ use: false }" :up="{ onScroll: true }" @up="upCallback" @init="mescrollInit">
			...
			<view class="mb18r bgwhite">
				...
				<view class="posir">
					...
					<view class="album-name">{{ albumInfoData.dynamic_title }}</view>
				</view>
				<!-- 用户头部 -->
				...
				<view class="flexr-jfe flex-aic ptb18r plr18r bts2r br8r" v-if="topListData">
					...
					<view class="f24r c111 fcenter bgf8 w128r ptb18r" @tap="fnTopList">
						赞
						<text class="f24r cbrown ml18r">{{ albumInfoData.TopCount }}</text>
					</view>
				</view>
			</view>
			<!-- 评论区 -->
		...
		<view class="posif posi-blr0 flexr-jsa plr18r ptb18r bts2r z5 bgwhite">
			...
			<view class="plr28r bls2r brs2r" @tap="fnTop">
				...
				<text class="f28r cgray ml8r">{{ albumInfoData.cache_extends ? albumInfoData.cache_extends.praises_count : 0 }}</text>
			</view>
			<view class="plr28r" @tap="fnSave">
				<i-icon type="shoucang" size="48" :color="albumInfoData.is_collection ? '#FF6699' : '#8F8F94'"></i-icon>
				<text class="f28r cgray ml8r">{{ albumInfoData.cache_extends ? albumInfoData.cache_extends.collection_count : 0 }}</text>
			</view>
		...
		upCallback(mescroll) {
			...
			if (mescroll.num == 1) {
				...
				getDynamicInfo(this.dynamic_id)
					.then(albumRes => {
						this.$store.commit('album/setAlbumInfoData', albumRes.data);
						// 导航标题
						uni.setNavigationBarTitle({
							title: albumRes.data.dynamic_title
						});
						params.count = 8;
						// 获取点赞列表8项
		...
		fnTop() {
			...
			dynamicPraise(filItem.dynamic_id).then(res => {
				...
				if (filItem.is_praise) {
					...
					this.albumInfoData.cache_extends.praises_count--;
					// 点赞列表减头像
