	...
	<view>
			...
			<view class="mb18r bgwhite">
				...
				<view class="flexr-jfe flex-aic ptb18r plr18r bts2r br8r" v-if="topListData">
					...
					<view class="f24r c111 fcenter bgf8 w128r ptb18r" @tap="fnTopList">
						...
						<text class="f24r cbrown ml18r">{{ videoInfoData.cache_extends ? videoInfoData.cache_extends.praises_count : 0}}</text>
					</view>
				</view>
			</view>
			<!-- 评论区 -->
			<view class="plr18r ptb28r f32r fbold c111 bbs2r bgwhite">评论（{{ videoInfoData.cache_extends ? videoInfoData.cache_extends.comments_count : 0 }}）</view>
		...
		<view class="posif posi-blr0 flexr-jsa plr18r ptb18r bts2r z5 bgwhite">
			...
			<view class="plr28r bls2r brs2r" @tap="fnTop">
				...
				<text class="f28r cgray ml8r">{{ videoInfoData.cache_extends ? videoInfoData.cache_extends.praises_count : 0 }}</text>
			</view>
	...
	methods: {
		...
		fnTop() {
			...
			dynamicPraise(filItem.dynamic_id).then(res => {
				...
				if (filItem.is_praise) {
					...
					this.videoInfoData.cache_extends.praises_count--;
					// 点赞列表减头像
		...
		fnSave() {
			...
			dynamicCollection(filItem.dynamic_id).then(res => {
				...
				if (filItem.is_collection) {
					...
					this.videoInfoData.cache_extends.collection_count--;
				} else {
					filItem.cache_extends.collection_count++;
		...
		fnCommSend(e) {
			...
			addComment(e).then(res => {
				...
				// 评论数量添加
				if (this.videoInfoData.cache_extends.comment_count == 0) this.mescroll.removeEmpty();
				this.videoInfoData.cache_extends.comment_count++;
					...
					switch (res.tapIndex) {
						...
						case 3:
							delComment(e.comment_id).then(res => {
								...
								if (this.videoInfoData.cache_extends.comment_count == 0) this.mescroll.showEmpty();
								// 改变上一窗口的数据
								let filItem = {};
								// 来自主要跳转
								if (this.fromPage == 'home') {
