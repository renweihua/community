<template>
  <view class="mescroll-uni-warp">
    <scroll-view :id="viewId" class="mescroll-uni" :class="{'mescroll-uni-fixed':fixed}" :style="{'padding-top':padTop,'padding-bottom':padBottom,'top':fixedTop,'bottom':fixedBottom}"
      ...
      @touchmove="touchmoveEvent" @touchend="touchendEvent" @touchcancel="touchendEvent" :scroll-y='scrollAble'
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      fixedTop() {
        let num = (this.numTop + this.windowTop);
        // #ifdef APP-PLUS
        num += (num < 60 && num > 42) ? uni.getSystemInfoSync().statusBarHeight : 0
        // #endif 
        // #ifdef H5
        num = (num <= 50 || num >= 200) ? num : num / 2
        // #endif
        return this.fixed ? num + 'px' : 0
