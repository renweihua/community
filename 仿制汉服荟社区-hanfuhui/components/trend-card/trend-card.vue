<template>
    <view class="bgwhite mb18r">
        <!-- 头部显示信息含时间  -->
        <view class="flexr-jsb flex-aic plr18r ptb18r" v-if="infoData" :id="infoData.user_info.dynamic_id">
            <view class="flex" @tap="$_click('user')">
                <user-avatar :src="calUserAvater" :tag="infoData.user_info.user_uuid" size="md"></user-avatar>
                <view class="flexc-jsa ml28r">
                    <view>
                        <text class="f28r fbold mr18r">{{infoData.user_info.nick_name}}</text>
                        <i-icon :type="infoData.user_info.user_sex_text == '男' ?'nan':'nv' " size="28"
        ...
        <view class="plr18r pb18r">
            ...
            <view class="flex flex-fww">
                <block v-if="infoData.topic" :key="infoData.topic.topic_id">
                    <text class="huiba-tag mr18r mt18r" @tap="$emit('huiba', infoData.topic)">
                        {{infoData.topic.topic_name}}
                    </text>
                </block>
            </view>
        </view>
        ...
        <view class="flexr-jsa bts2r">
            <view class="trend-w20v hl80r fcenter" @tap="$_click('top')">
                <i-icon type="dianzan" size="48" :color="infoData.is_prise?'#FF6699':'#8f8f94'"></i-icon>
                <text class="ml8r f28r cgray">{{infoData.praise_count || '赞'}}</text>
            </view>
            <view class="trend-w20v hl80r fcenter" @tap="$_click('comm')">
                <i-icon type="pinglun" size="48" color="#8f8f94"></i-icon>
                <text class="ml8r f28r cgray">{{infoData.reply_count || '评论'}}</text>
            </view>
            <view class="trend-w20v hl80r fcenter" @tap="$_click('save')">
                <i-icon type="shoucang" size="48" :color="infoData.is_collection?'#FF6699':'#8f8f94'"></i-icon>
                <text class="ml8r f28r cgray">{{infoData.collection_count || '收藏'}}</text>
            </view>
        </view>
    </view>
</template>
...
<script>
    ...
    export default {
        ...
        computed: {
            /// 计算显示用户头像
            calUserAvater() {
                return this.infoData.user_info.user_head ? this.infoData.user_info.user_head : '/static/default_avatar.png'
            },
            /// 计算摄影信息
            calAlbum() {
                if (this.infoData.object_type != 'album') return false
                let albumInfo = []
                let data = JSON.parse(this.infoData.ObjectData)
            ...
            /// 计算显示视频封面
            calVideoCover() {
                if (this.infoData.object_type != 'video') return false
                let cover = '/static/default_image.png'
                let suffix = '_850x300.jpg/format/webp'
                if (this.infoData.dynamic_imgs) cover = this.infoData.dynamic_imgs[0] + suffix
                return cover
            },
            /// 计算显示图片格
            calImageSrcs() {
                let imgArray = this.infoData.dynamic_imgs || [];
                if (imgArray) {
            ...
            /// 计算内容截取字符长度 180
            calStringCut() {
                let content = this.infoData.dynamic_title + ''; //转成字符串
            ...
            /// 计算标题字符转json
            calTitle() {
                if (['topic', 'topicreply', 'word'].includes(this.infoData.object_type)) {
                    let topic = JSON.parse(this.infoData.ObjectData)
        ...
        methods: {
            ...
            fnPreviewImage(current) {
                let urls = this.infoData.dynamic_imgs.map(url => url)
                previewImage(current, urls)
            },
            /// 更多菜单操作
            async fnActionSheet() {
                let atteTitle = this.infoData.user_info.is_follow ? '取消关注' : '关注TA';
                // this.infoData.user_friend.is_black 一直为false 这里用网络来判断存在咯
                let blackRes = await getUserExistsBlack(this.infoData.user_info.user_id);
                this.infoData.user_friend.is_black = blackRes.data.data;
                // 黑名单状态
                let blackTitle = this.infoData.user_friend.is_black ? '移出黑名单' : '将TA拉黑';
                let blackContent = this.infoData.user_friend.is_black ? '是否将该用户移出黑名单' : '拉黑后再广场将看不到TA的动态，TA将无法对你发消息、评论。';
                uni.showActionSheet({
                    itemList: [atteTitle, blackTitle, '举报'],
                    success: res => {
                        if (res.tapIndex == 0) return this.$emit('follow', this.infoData)
                        if (res.tapIndex == 2) return this.$emit('report', this.infoData)
                        if (res.tapIndex == 1) {
                            uni.showModal({
                                content: blackContent,
                                success: res => {
                                    if (res.confirm) {
                                        this.$emit('black', this.infoData)
                                    }
                                }
                            });
                            return
